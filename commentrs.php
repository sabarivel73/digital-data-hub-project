...
<html>
    ...
    <body>
        <div class="container">
            <h1>View user comments about Road Safety Rules & Regulations</h1>
            <nav>
                <ul>
                    <button><a href="rs.php">Back</a></button>
                </ul>
            </nav>
        </div>
          <div class="main">
           <div class="content">
            <?php
                $sql="SELECT user.UNAME,comment3.COMM,comment3.LOGS FROM comment3 INNER JOIN user on comment3.UID=user.UID ORDER BY comment3.CID DESC";
                $res=$db->query($sql);
                if($res->num_rows>0)
                {
