<?php
    ...
    if(isset($_POST['submit']))
    {
        ...
        if($cc=="Yes")
        {
            $ccError=$cc." (According to Indian Government Law if you have any criminal case not allowed for DL)";
        }
        else
        {
        ...
        if($lap=="No")
        {
            $lapError=$lap." (Aadhaar must be linked with Phone number for Online DL)";
        }
        else
        {
            $lapError=$lap." (You are Eligible for Online DL)";
        }
        if($wm=="No")
        {
            $wmError=$wm." (First apply LLR and then apply for DL one month after from the applying date of LLR)";
        }
        else
        {
            $wmError=$wm." (You are Eligible)";
        }
        if($age>=18 && $cc=="No" && $lap=="Yes" && $wm=="Yes")
        {
            $allSuccess="Your are eligible for DL";
        }
        else
        {
          ...
          <div class="navbar">
                <nav>
                    <ul>
                        ...
                        <button><a href="eligibledl.php">Refresh</a></button>
       ...
       <div class="main">
        <div class="content">
          <form action="" method="post">
                ...
                <br>
                <label>did you apply for LLR :</label>
                <select name="wm" required>
                    <option vaalue="">Select</option>
                    <option vaalue="oyes">Yes</option>
                    <option vaalue="ono">No</option>
                </select>
                <br>
                <span><?php echo $wmError ?></span>
                <br>
