...
<html>
    ...
    <body>
        <div class="container">
            <h1>View user comments about LLR</h1>
            <nav>
                <ul>
                    <button><a href="llrhome.php">Home</a></button>
                </ul>
            </nav>
        </div>
          <div class="main">
           <div class="content">
            <?php
                $sql="SELECT user.UNAME,comment2.COMM,comment2.LOGS FROM comment2 INNER JOIN user on comment2.UID=user.UID ORDER BY comment2.CID DESC";
                $res=$db->query($sql);
                if($res->num_rows>0)
                {
